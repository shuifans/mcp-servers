"""``retrieve_aliyun_docs`` tool implementation.

Composes :func:`search_aliyun_docs` + :func:`read_aliyun_doc` into a list of
:class:`~mcp_servers.aliyun_help_docs.core.evidence.Evidence` objects.
"""
...
async def retrieve_aliyun_docs(
    query: str,
    ...
    *,
    ...
    try:
        search_result = await search_aliyun_docs(
            query=query,
            product=product,
            top_k=top_k_chunks,
            client=iqs,
        ...
        async def _read(item: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
            async with sem:
                doc = await read_aliyun_doc(item.get("url") or item.get("link") or "", cache=cache_inst, client=iqs)
                return item, doc
        ...
        for item, doc in reads:
            if not isinstance(doc, dict) or doc.get("error"):
                logger.info("skip candidate due to read error: %s", doc.get("error") if isinstance(doc, dict) else doc)
                continue
