...
- ``help.aliyun.com`` (any path)
- ``www.alibabacloud.com`` but only when path starts with ``/help``
...
- Configured via the ``URL_WHITELIST`` env var (comma separated).
"""
...
class _Rule:
    ...
    def matches(self, host: str, path: str) -> bool:
        ...
        return normalized_path == prefix or normalized_path.startswith(prefix + "/") or normalized_path.startswith(self.path_prefix)


def _load_rules(whitelist: Iterable[str] | None) -> list[_Rule]:
    if whitelist is None:
        whitelist = get_config().url_whitelist
    rules: list[_Rule] = []
...
def validate_url(url: str, whitelist: Iterable[str] | None = None) -> None:
...
def is_url_allowed(url: str, whitelist: Iterable[str] | None = None) -> bool:
